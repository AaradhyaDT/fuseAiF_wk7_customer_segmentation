# AARADHYA DEV TAMRAKAR — Master Profile v90

*Last updated: June 12, 2026 — v90: Fusemachines Week 7 Clustering Assignment logged (due June 21, 2026; UCI Online Retail II dataset; K-Means + Hierarchical + DBSCAN pipeline; full task plan `Week7_Clustering_TaskPlan.md` generated; skills added: clustering algorithms, silhouette/Davies-Bouldin/Calinski-Harabasz validation metrics, RFM feature engineering, DBSCAN parameter tuning). Today's exam: O&M Internal Test 1 (June 12, Jestha 29).*
...
- **Pipeline:** Raw transactions → RFM feature engineering (+ extended: AvgBasketSize, AvgDaysBetweenPurchases, UniqueProducts, ReturnRate) → preprocessing (outlier handling + StandardScaler) → K-Means (Elbow + Silhouette, k-means++ vs random init) → Hierarchical (Ward + Complete linkage, dendrogram cut) → DBSCAN (k-distance ε estimation, ≥3 param combos, noise analysis) → validation (Silhouette / Davies-Bouldin / Calinski-Harabasz) → business narrative (cluster profiles + executive summary) → failure log (≥3 entries).
- **Task plan generated:** `Week7_Clustering_TaskPlan.md` (full section-by-section breakdown with code hints, justification templates, pacing schedule).
- **Submission:** Single `.ipynb` with all cells executed.
- **Skills added to stack:** K-Means (inertia, elbow, init strategies) · Agglomerative Clustering · DBSCAN (ε/min_samples tuning, noise analysis) · Silhouette Score · Davies-Bouldin Index · Calinski-Harabasz Index · RFM feature engineering.
...
| 11 | Fusemachines Week 7 Clustering Assignment | — | **In Progress** — due June 21, 2026; task plan generated; UCI Online Retail II; K-Means + Hierarchical + DBSCAN |
